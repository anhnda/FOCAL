...
Length-preserving ablation: dropped input tokens are REPLACED with a
neutral mask token (a Llama 3 reserved special token) rather than removed,
so absolute token positions of kept tokens, the suffix, and the answer
never shift across coalitions.
...
def get_mask_token_id(tokenizer):
    """Resolve a neutral 'mask' token id for length-preserving ablation.

    Llama 3 doesn't have a dedicated <mask> token (it's a causal LM, not
    BERT-style). The reserved special tokens were never trained on real
    text and are the closest thing to an unused slot. We try those first,
    fall back to the finetune pad id, then eos as a last resort.
    """
    candidates = [
...
def assemble_from_mask(template_prefix_ids, template_suffix_ids,
                       flat_input_ids, mask, mask_token_id):
    """Length-preserving: dropped positions are replaced with mask_token_id
    rather than removed, so absolute positions of kept tokens, the suffix,
    and the answer never shift across coalitions."""
    keep = mask.bool()
    replaced = torch.where(
        keep,
        flat_input_ids,
        torch.full_like(flat_input_ids, mask_token_id),
    )
    return torch.cat([template_prefix_ids, replaced, template_suffix_ids])
...
def sweep(model, template_prefix_ids, template_suffix_ids,
          flat_input_ids, answer_ids, masks, mask_token_id,
          top_k=TOP_K_ALTS):
    k = masks.shape[0]
    T = answer_ids.numel()
    ...
    expected_len = (template_prefix_ids.numel() + flat_input_ids.numel()
                    + template_suffix_ids.numel())
    ...
    for j in range(k):
        prompt_S = assemble_from_mask(template_prefix_ids,
                                      ...
                                      mask_token_id)
        assert prompt_S.numel() == expected_len, (
            f"Length-preserving invariant violated: "
            f"got {prompt_S.numel()}, expected {expected_len}"
        )
...
def explain_question(model, tokenizer, parts, stop_token_ids, mask_token_id):
    ...
    print(f"  Running {k} forward passes (Bernoulli masks, "
          f"length-preserving, no LOO/full/empty rows) ...")
...
def main():
    ...
    print(f"Using mask token id={mask_token_id} "
          f"({tokenizer.decode([mask_token_id])!r}) for length-preserving "
          f"ablation.")
    ...
    for q in questions:
        explain_question(model, tokenizer, q, stop_token_ids, mask_token_id)
