...
Benchmark Owen-Value (Hierarchical / Partition Shapley) Attribution on
decoder-only GPT-2 models using the TellMeWhy dataset loaded from a
local raw-text file.

Mirrors `run_eval_fd_gpt2.py` exactly in structure, dataset loader, and
metric reporting (Soft-NC / Soft-NS / Log-odds — Zhao & Shan, ReAGent
AAAI 2024). The only differences are:

  - import shapowen instead of fd_gpt2
  - call shap_owen_gpt2 (no integration path, no autograd)
  - drop the --steps flag (Owen is deterministic and has no path length)

Usage:
    python eval_shapowen.py --model_name gpt2 --num_samples 200
    python eval_shapowen.py --model_name gpt2-medium --use_gold --verbose
    python eval_shapowen.py --baseline pad --eval-baseline pad
"""
...
from shapowen import shap_owen_gpt2, get_model_tokenizer, _build_base_embed
...
# ─────────────────────────────────────────────────────────────────────────────
# Dataset loader (identical to run_eval_fd_gpt2.py)
# ─────────────────────────────────────────────────────────────────────────────
...
def run_benchmark(args) -> None:
    ...
    print(f"Method         : Owen-Value (Hierarchical Shapley), no autograd")
    ...
    print("\n" + "=" * 60)
    print("FINAL RESULTS — Owen-Shapley / GPT-2")
    print("=" * 60)
    if count > 0:
...
def _print_sample(question: str, res: dict):
    ...
    print("Top-5 Q tokens by |Owen attribution|:")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Benchmark Owen-Shapley on GPT-2 + TellMeWhy"
    )
    parser.add_argument(
        "--data_path", type=str,
    ...
    parser.add_argument(
        ...
        help="Max batch size for a single forward pass (default: 32). "
             "Total forwards: O(Lq) coalitions, typically 2*Lq − 1.",
    )

    # --- Metric / generation knobs (match run_eval_fd_gpt2.py) ------------
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Owen baseline embedding: zero | pad | mean (default: zero)",
    )
    parser.add_argument(
