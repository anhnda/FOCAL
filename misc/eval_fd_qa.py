"""
Benchmark Script for Forward-Only PACE (FD) Attribution on Question Answering

Usage:
    python eval_fd_qa.py --model_name deepset/bert-base-cased-squad2 --num_samples 1000 --steps 101

Same CLI as run_eval_pg_qa but uses fd.py (forward-only, no autograd).
"""
import time
import random
import argparse
...
from fd import pace_gradient_qa, get_model_tokenizer, get_baseline_embedding
...
def run_single_example(question, context, model_name,
                       a, b, steps, device,
                       baseline, eval_base_token_emb,
                       chunk_size,
                       topk=20):
...
def run_benchmark(args):
    ...
    print(f"FD baseline   : {args.baseline}")
    print(f"Eval baseline : {args.eval_baseline}")
    print(f"Chunk size    : {args.chunk_size}")
    print(f"Method        : Forward-Only (FD), no autograd")
    ...
    if count > 0:
        print(f"\n{'─'*52}")
        print(f"FD (Forward-Only PACE) QA  |  {args.model_name}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Benchmark Forward-Only PACE (FD) for QA"
    )
    ...
    parser.add_argument("--baseline",      type=str, default="mask",
                        ...
                        help="Baseline embedding for FD integration path")
    ...
    parser.add_argument("--chunk-size",    type=int, default=64,
                        help="Max batch size per forward pass (memory/throughput tradeoff)")
