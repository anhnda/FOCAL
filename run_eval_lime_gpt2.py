"""
run_eval_lime_gpt2.py

Benchmark LIME Token Attribution on decoder-only GPT-2 models using the
TellMeWhy dataset loaded from a local raw-text file.

Mirrors `run_eval_pg_gpt2.py` exactly in structure, dataset loader, and
metric reporting (Soft-NC / Soft-NS / Log-odds following Zhao & Shan,
ReAGent AAAI 2024). The only differences are:

  - import lime_gpt2 instead of paceg_gpt2
  - swap the attribution-method call (steps -> n_samples, etc.)
  - add LIME-specific CLI flags: --p_keep, --sigma, --ridge_lambda

Usage:
    python run_eval_lime_gpt2.py --model_name gpt2 --num_samples 200 --n_lime 1000
    python run_eval_lime_gpt2.py --model_name gpt2-medium --use_gold --verbose
"""
...
from lime_gpt2 import lime_gpt2, get_model_tokenizer, _build_base_embed
...
def run_benchmark(args) -> None:
    ...
    print(f"LIME N         : {args.n_lime}")
    print(f"LIME p_keep    : {args.p_keep}")
    print(f"LIME sigma     : {args.sigma}")
    print(f"LIME ridge λ   : {args.ridge_lambda}")
    print(f"LIME chunk     : {args.chunk_size}")
    ...
    for idx, sample in enumerate(tqdm(samples, desc="Evaluating")):
        try:
            res = run_single_example(
                ...
                seed            = args.seed + idx,    # vary seed across samples
                eval_base_embed = eval_base_embed,
            )
    ...
    print("\n" + "=" * 60)
    print("FINAL RESULTS — LIME / GPT-2")
    print("=" * 60)
    if count > 0:
...
def _print_sample(question: str, res: dict):
    ...
    print("Top-5 Q tokens by |LIME coefficient|:")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Benchmark LIME Token Attribution on GPT-2 + TellMeWhy"
    )
    parser.add_argument(
        "--data_path", type=str,
    ...
    # --- LIME-specific ----------------------------------------------------
    parser.add_argument(
        "--n_lime", type=int, default=1000,
        help="LIME number of binary masks N (default: 1000)",
    )
    parser.add_argument(
        "--p_keep", type=float, default=0.5,
        help="Bernoulli keep-probability for free Q tokens (default: 0.5)",
    )
    parser.add_argument(
        "--sigma", type=float, default=0.25,
        help="Kernel bandwidth in cosine-distance space (default: 0.25)",
    )
    parser.add_argument(
        "--ridge_lambda", type=float, default=1.0,
        help="L2 penalty for the ridge surrogate (default: 1.0)",
    )
    parser.add_argument(
        "--chunk_size", type=int, default=32,
        help="Batch size for masked forward passes (default: 32)",
    )
    parser.add_argument(
        "--seed", type=int, default=42,
        help="Base seed for LIME's mask sampler (varied per sample)",
    )
    ...
    parser.add_argument(
        ...
        help="LIME baseline embedding: zero | pad | mean (default: zero)",
    )
    parser.add_argument(
