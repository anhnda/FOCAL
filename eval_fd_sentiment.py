"""
Benchmark Script for Forward-Only PACE (FD) Attribution on Classification

Usage:
    python eval_fd_sentiment.py --model distilbert --dataset sst2 --steps 100

Same CLI as eval_pg_sentiment but uses fd.py (forward-only, no autograd).
"""
import time
...
from fd import pace_gradient_classification, get_baseline_embedding
...
if __name__ == "__main__":
    ...
    print(f"FD baseline   : {baseline}")
    print(f"Eval baseline : {eval_baseline}")
    print(f"Range         : [{a}, {b}]  steps={steps}  chunk={chunk_size}")
    ...
    print_step = 100
    print("\nStarting FD attribution computation...")
